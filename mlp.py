...
class MLP:
    ...
    def __init__(self, sizes, beta=2, momentum=0.6):
        """
        ...
        self.nout = sizes[
            3]  # number of classes / the number of neurons in the output layer

        # Initialise the network of two hidden layers
        self.weights1 = (np.random.rand(self.nin + 1, self.nhidden1) -
                         0.5) * 2 / np.sqrt(self.nin)  # hidden layer 1
        self.weights2 = (np.random.rand(self.nhidden1 + 1, self.nhidden2) -
                         0.5) * 2 / np.sqrt(self.nhidden1)  # hidden layer 2
        self.weights3 = (np.random.rand(self.nhidden2 + 1, self.nout) -
                         0.5) * 2 / np.sqrt(self.nhidden2)  # output layer
    ...
    def train(self, inputs, targets, eta, niterations):
        ...
        for n in range(niterations):
            ...
            deltao = (self.outputs - targets) * self.outputs * (1 -
                                                                self.outputs)

            # compute the derivative of the second hidden layer
            deltah2 = self.beta * self.ds(self.hidden2, deltao, self.weights3)
            ...
            updatew1 = self.uw(updatew1, inputs, deltah1, eta, self.momentum)
            updatew2 = self.uw(updatew2, self.hidden1, deltah2, eta,
                               self.momentum)
            updatew3 = eta * np.dot(np.transpose(self.hidden2),
                                    deltao) + self.momentum * updatew3
    ...
    def forwardPass(self, inputs):
        ...
        bias1 = -np.zeros((np.shape(inputs)[0], 1))
        self.hidden1 = self.sf(self.hidden1)
        self.hidden1 = np.concatenate((self.hidden1, bias1), axis=1)
        # layer 2
        ...
        bias2 = -np.zeros((np.shape(self.hidden1)[0], 1))
        self.hidden2 = self.sf(self.hidden2)
        self.hidden2 = np.concatenate((self.hidden2, bias2), axis=1)
        # output layer
        # compute the forward pass on the output layer with softmax function
        outputs = self.sof(np.dot(self.hidden2, self.weights3))
    ...
    def ds(self, a, delta, b):
        delta_ = self.beta * a * (1.0 - a) * (np.dot(delta, np.transpose(b)))
        return delta_

    def uw(self, updatew, a, delta, lr, momentum):
        updatew = lr * np.dot(np.transpose(a),
                              delta[:, :-1]) + momentum * updatew
        return updatew
    ...
    def sof(self, a):
        len = np.shape(a)[0]
        N = np.sum(np.exp(a), axis=1) * np.ones((1, len))
        a = np.exp(a)
        a = np.transpose(a) / N
        a = np.transpose(a)
        return a
